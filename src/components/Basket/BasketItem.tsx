...
export const BasketItem = (props: {
	offer: TProduct & { quantity: number }
}) => {
	const {
		addOfferToBasket,
		...
		<div className="basket-item">
			<div className="basket-item__visual">
				...
				<div className="counter-menu">
					...
					<button
						className="counter-menu__button"
						onClick={() => removeOneOfferFromBasket(offer.id)}
					>
						{/* короткое тире */}–
					</button>
					...
					<button
						className="counter-menu__button"
						onClick={() => addOfferToBasket(offer.id)}
					>
						+
					</button>
				</div>
			...
			<div className="basket-item__info">
				<div className="product-item__h3">{offer.title}</div>
				<span className="price-text">{`${offer.price} ₽`}</span>
			</div>
			...
			<button
				onClick={() => removeOfferFromBasket(offer.id)}
